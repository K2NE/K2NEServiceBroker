...
namespace K2Field.K2NE.ServiceBroker.Helpers.PowerShell
{
    ...
    {
        public static string RunScriptFile(string fileLocation, List<PowerShellVariablesDC> variablesList)
        {
            string fileContent = LoadScriptByPath(fileLocation);
            return RunScriptCode(fileContent, variablesList);
        }
        ...
        private static string LoadScriptByPath(string filePath)
        {
            try
            {
        ...
        public static Dictionary<string, string> GetFilePathsFromDirectories(string folderNames)
        {
            ...
            foreach (string folderName in folderNames.Split(';'))
            {
                ...
                else
                {
                    ...
                    {
                        throw new Exception(string.Format("The PowerShell directory '{0}' could not be created: {1}", fullFolderPath, e.Message), e);
                    }
                }
            }
