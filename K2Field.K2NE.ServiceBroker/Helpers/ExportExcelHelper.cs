    ...
    public class CreateExcel : MarshalByRefObject
    {
        ...
        private void CreateWorkSheetPart(WorksheetPart worksheetPart, DataTable table)
        {
            ...
            for (int rIndex = 0; rIndex < table.Rows.Count; rIndex++)
            {
                ...
                {
                    Cell cell = new Cell();
                    cell.CellReference = new CreateExcel().Get(cIndex, (Convert.ToInt32((UInt32)rowIndex) - 2), this.cellHeaders);
                    ...
                    // {
                    //     cell.StyleIndex = GetCellStyle(table.Rows[rIndex][cIndex].GetType().ToString());
                    // }
                    ...
                    if (table.Rows[rIndex][cIndex].GetType().ToString() == "System.DateTime")
                    {
                        cellValue.Text = Convert.ToDateTime(table.Rows[rIndex][cIndex].ToString()).ToOADate().ToString();
        ...
        private UInt32Value GetCellStyle(string col)
        {
            switch (col)
            {
                case "System.DateTime":
