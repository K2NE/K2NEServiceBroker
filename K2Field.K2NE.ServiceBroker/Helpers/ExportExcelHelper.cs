...
{
    [Serializable]
    public class CreateExcel : MarshalByRefObject, IDisposable
    {
        ...
        private SmartObjectClientServer cServer = null;
        ...
        private void Check_SmartObjectClientServer()
        {
            try
            {
                if (cServer == null)
                {
                    cServer = new SmartObjectClientServer();
                    cServer.CreateConnection();
                    cServer.Connection.Open(cStr);
                }
            }
            catch
            {
        ...
        public string GetExcelFromADOQuery(string query, string fname)
        {
            ...
            try
            {
                using (SOConnection connection = new SOConnection(cStr))
                ...
                new CreateExcel().ExportToExcel(results, filePath, cellHeaders);

                FileStream file = new FileStream(filePath, FileMode.Open, FileAccess.Read);
                byte[] bytes = new byte[file.Length];
                file.Read(bytes, 0, (int)file.Length);

                string tmpFile = new FileObject(filename, Convert.ToBase64String(bytes, 0, (int)file.Length, Base64FormattingOptions.None)).ToString();
            ...
            {
                throw new Exception("Error with ADO Query : '" + query + "'  Msg : " + ex.Message);
            }
        }
        ...
        public string GetExcelFromADOQuery(DataTable results, string fname)
        {
            ...
            {
                throw new Exception("Error with ADO Query -  Msg : " + ex.Message);
            }
        }
        ...
        public void ExportToExcel(DataTable datatable, string filepath, List<string> cellheaders)
        {
            ...
            //SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(filepath,
        ...
        {
            if (cServer != null && cServer.Connection.IsConnected)
            {
